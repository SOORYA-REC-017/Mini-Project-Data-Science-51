...
# Correct TensorFlow and Keras imports
...
def CNN2D(X_train, X_test, y_train, y_test):
    ...
    week2 = [np.reshape(row, (-1, 7, 1)) for row in n_array_X_test_extended]
    ...
    # Model creation
    model = Sequential()
    model.add(Conv2D(filters=32, kernel_size=(7, 3), input_shape=input_shape, activation='relu'))
    model.add(Flatten())
    model.add(Dense(1, activation='sigmoid'))
